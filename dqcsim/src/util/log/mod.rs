...
/// log records to a thread local logger. If no thread local logger is set using
/// the `[dqcsim::util::log::set_thread_logger]` function, log records are discarded.
///
...
/// Connects to a log thread receiver server via an IPC channel.
///
