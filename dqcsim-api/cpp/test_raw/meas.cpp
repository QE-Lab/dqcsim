...
TEST(meas, test) {
  ...
  EXPECT_EQ(dqcs_meas_new(0, dqcs_measurement_t::DQCS_MEAS_ONE), 0);
  EXPECT_STREQ(dqcs_error_get(), "Invalid argument: 0 is not a valid qubit reference");

  EXPECT_EQ(dqcs_meas_new(1, dqcs_measurement_t::DQCS_MEAS_INVALID), 0);
  ...
  // Check the contents.
  EXPECT_EQ(dqcs_meas_qubit_get(a), 2);
  ...
  // Mutate the contents.
  EXPECT_EQ(dqcs_meas_qubit_set(a, 5), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_error_get();
  ...
  // Check the contents.
  EXPECT_EQ(dqcs_meas_qubit_get(a), 5);
  ...
  EXPECT_EQ(dqcs_meas_qubit_set(a, 0), dqcs_return_t::DQCS_FAILURE);
  EXPECT_STREQ(dqcs_error_get(), "Invalid argument: 0 is not a valid qubit reference");
  ...
  // Check the contents.
  EXPECT_EQ(dqcs_meas_qubit_get(a), 0);
  EXPECT_EQ(dqcs_error_get(), "Invalid argument: handle " + std::to_string(a) + " is invalid");
