...
TEST(json, string) {
  ...
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: handle 0 is invalid");

  // Check improper object.
  EXPECT_EQ(dqcs_arb_json_set_str(a, "invalid JSON"), dqcs_return_t::DQCS_FAILURE);
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: expected value at line 1 column 1");
...
TEST(args, test1) {
  ...
  EXPECT_EQ(dqcs_arb_push_str(a, "First argument"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_push_str(a, "2nd argument"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_push_str(a, "3rd argument"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_push_str(a, ")*#$()&#$"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_push_str(a, ""), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  ...
  // Push some data.
  int i = 42*33;
  EXPECT_EQ(dqcs_arb_push_raw(a, &i, sizeof(i)), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_push_raw(a, nullptr, 0), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();

  // Pushing null with nonzero length is not okay.
  EXPECT_EQ(dqcs_arb_push_raw(a, nullptr, 10), dqcs_return_t::DQCS_FAILURE);
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: unexpected NULL data");
  ...
  ASSERT_NE(b, 0) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_assign(b, a), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  ...
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: unexpected NULL buffer");
  ...
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: pop from empty list");
  ...
  // Try a no-return pop.
  EXPECT_EQ(dqcs_arb_pop(b), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  ...
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: nul byte found in provided data at position: 2");
  ...
  EXPECT_EQ(dqcs_arb_clear(b), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
...
TEST(args, test2) {
  ...
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: index out of range: 0");
  EXPECT_EQ(dqcs_arb_get_str(a, -1), nullptr);
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: index out of range: -1");
  EXPECT_EQ(dqcs_arb_get_str(a, 1), nullptr);
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: index out of range: 1");
  ...
  EXPECT_EQ(dqcs_arb_insert_str(a, 0, "hi"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_len(a), 1);
  EXPECT_EQ(dqcs_arb_clear(a), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_insert_str(a, -1, "hi"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_len(a), 1);
  EXPECT_EQ(dqcs_arb_clear(a), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();

  // But not at -2 or 1.
  EXPECT_EQ(dqcs_arb_insert_str(a, -2, "hi"), dqcs_return_t::DQCS_FAILURE);
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: index out of range: -2");
  EXPECT_EQ(dqcs_arb_insert_str(a, 1, "hi"), dqcs_return_t::DQCS_FAILURE);
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: index out of range: 1");

  // Insert some stuff using positive indices.
  EXPECT_EQ(dqcs_arb_insert_str(a, 0, "2"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_insert_str(a, 1, "4"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_insert_str(a, 0, "1"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();

  // And some using negative indices.
  EXPECT_EQ(dqcs_arb_insert_str(a, -2, "3"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_insert_str(a, -1, "5"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_insert_str(a, -6, "0"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  ...
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: index out of range: 6");
  EXPECT_EQ(dqcs_arb_remove(a, -7), dqcs_return_t::DQCS_FAILURE);
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: index out of range: -7");

  // Check that remove works as expected.
  EXPECT_EQ(dqcs_arb_remove(a, 2), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_remove(a, 0), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_remove(a, -2), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_remove(a, 3), dqcs_return_t::DQCS_FAILURE);
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: index out of range: 3");
  EXPECT_EQ(dqcs_arb_remove(a, -4), dqcs_return_t::DQCS_FAILURE);
  EXPECT_STREQ(dqcs_explain(), "Invalid argument: index out of range: -4");
  ...
  EXPECT_EQ(dqcs_arb_set_str(a, 0, "hello"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_set_str(a, -1, "world"), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  EXPECT_EQ(dqcs_arb_set_str(a, 1, ", "), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
...
TEST(args, test3) {
  ...
  EXPECT_EQ(dqcs_arb_insert_raw(a, 0, &value, sizeof(value)), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
  ...
  EXPECT_EQ(dqcs_arb_insert_raw(a, 0, &value2, sizeof(value2)), dqcs_return_t::DQCS_SUCCESS) << "Unexpected error: " << dqcs_explain();
